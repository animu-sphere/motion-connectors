                     ...
                     const mocopi::PacketCapture* provenance) const
{
    if (receiver)
    {
        ...
        std::fprintf(out, "listen:      %s%s, receive buffer %zu bytes\n",
                     ...
                     receiver->IsLoopbackOnly() ? " (loopback only: no device can reach it)" : "",
    ...
    if (_datagrams >= 2)
    {
        ...
        else
        {
            ...
            const bool capped =
                _prefix.size() == kMaxPrefixBytes && _shortestDatagram > kMaxPrefixBytes;
            std::fprintf(out,
                         "prefix:      %s%zu byte(s) common to every "
                         "datagram:\n",
