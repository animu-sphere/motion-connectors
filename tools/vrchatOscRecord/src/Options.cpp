...
GetUsage()
{
    return "vrchat_osc_record - record a VRChat OSC session, and say what arrived\n"
           "\n"
             ...
             std::string* error)
{
    *showHelp = false;
    ...
    {
        *error = std::string(strandedFlag) + " configures the trace an export writes, so it needs "
                                             "--export-trace";
        return false;
    }
