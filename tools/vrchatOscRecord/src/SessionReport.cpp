...
void
SessionReport::ObserveDatagram(const std::string& peer, const std::uint8_t* bytes,
                     ...
                     const vrchatOsc::PacketCapture* provenance) const
{
    if (receiver)
    {
        ...
        std::fprintf(
            out, "listen:      %s%s, receive buffer %zu bytes\n",
            receiver->GetBoundEndpoint().c_str(),
            receiver->IsLoopbackOnly() ? " (loopback only: reachable from this machine alone)" : "",
    ...
    if (provenance)
    {
        ...
        const Field fields[] = {
            {"sender", &provenance->sender},     {"device", &provenance->device},
            {"sourceId", &provenance->sourceId}, {"listen", &provenance->listenEndpoint},
        ...
        for (const Field& field : fields)
        {
            ...
            std::fprintf(out, "%s%s=%s", printed == 0 ? "provenance:  " : " ", field.key,
                         field.value->c_str());
            ++printed;
        }
        if (printed == 0)
...
SessionReport::_PrintLengths(std::FILE* out) const
{
    ...
    std::fprintf(out, "lengths:     %zu distinct in %llu datagram(s)", _lengths.size(),
                 static_cast<unsigned long long>(_datagrams));
    ...
    std::vector<std::pair<std::size_t, std::uint64_t>> ordered(_lengths.begin(), _lengths.end());
    ...
    for (std::size_t i = 0; i < ordered.size(); ++i)
    {
        ...
        std::fprintf(out, "%s%llu of %zu", i == 0 ? "" : ", ",
                     static_cast<unsigned long long>(ordered[i].second), ordered[i].first);
        if (i == 0)
        {
...
SessionReport::_PrintPrefix(std::FILE* out) const
{
    ...
    const bool capped = _prefix.size() == kMaxPrefixBytes && _shortestDatagram > kMaxPrefixBytes;
