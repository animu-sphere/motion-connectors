...
transport, the [address inventory](include/motionConnectorVrchatOsc/AddressInventory.h),
...
[`TrackingSpace.h`](include/motionConnectorVrchatOsc/TrackingSpace.h) is where the
conversion lives, one layer up, and
[report 03](https://github.com/animu-sphere/usd-vrm-plugins/blob/main/docs/reports/motion/03-2026-08-30-vrchat-osc-tracking-space.md)
...
is more than the mocopi connector had — its protocol is documented nowhere — and it
...
The VMC and mocopi connectors both carry humanoid bone transforms: a
message names a bone, and mapping it onto `openstrata::motion::HumanJoint` is a lookup. This
...
([report 02](https://github.com/animu-sphere/usd-vrm-plugins/blob/main/docs/reports/motion/02-2026-08-30-vrchat-osc-address-inventory.md) §2).
...
same frames, which `motionConnectorVrchatOsc_frameAssembler` measures by running
...
[`motionConnectorTransport`](../motionConnectorTransport/README.md) **before** this
...
`openstrata::motion::` type is named in the library yet: §2 gives a
connector `motionCore`
...
references, so `motionConnectorVrchatOsc_tests` — which names nothing in
...
here: the VMC connector held the only OSC decoder in this repository and this
