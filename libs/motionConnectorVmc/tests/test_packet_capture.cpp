...
// four-byte boundary, a type-tag string, one big-endian float. It is here as a
// realistic payload, not as a decode -- nothing in this file interprets it.
std::vector<std::uint8_t>
TimeMessage()
{
    return {
        0x2f, 0x56, 0x4d, 0x43, 0x2f, 0x45, 0x78, 0x74, 0x2f, 0x54,
        0x00, 0x00, 0x2c, 0x66, 0x00, 0x00, 0x3d, 0xcc, 0xcc, 0xcd,
    };
}
...
TestTheWrittenLayoutIsTheDocumentedOne()
{
    ...
    // line's gutter stays in the same column as a full one's.
    const std::string expected = "!vmc-packet-capture 1\n"
...
TestRoundTripIsByteIdentical()
{
    ...
    capture.listenEndpoint = "0.0.0.0:39539";
    capture.peerEndpoint = "127.0.0.1:52001";
    capture.datagrams.push_back(Datagram(0.0, TimeMessage()));
    ...
    capture.datagrams.push_back(Datagram(0.033333, {}));
    capture.datagrams.push_back(
        Datagram(0.033333, {0x23, 0x62, 0x75, 0x6e, 0x64, 0x6c, 0x65, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    assert(parsed.datagrams[0].bytes == TimeMessage());
...
TestAGutterIsCheckedRatherThanSkipped()
{
    ...
    const std::string lying = "!vmc-packet-capture 1\n"
                              "\n"
                              "d 0.000000 4\n"
                              "  2f 56 4d 43  |/XXX|\n";
    PacketCapture capture;
    ...
    const std::string bare = "!vmc-packet-capture 1\n"
                             "\n"
                             "d 0.000000 4\n"
                             "  2f 56 4d 43\n";
    ...
    const std::string upper = "!vmc-packet-capture 1\n"
                              "\n"
                              "d 0.000000 4\n"
                              "  2F 56 4D 43  |/VMC|\n";
    PacketCapture parsedUpper;
    assert(Read(upper, &parsedUpper));
    assert(parsedUpper.datagrams[0].bytes == std::vector<std::uint8_t>({0x2f, 0x56, 0x4d, 0x43}));
    assert(Write(parsedUpper).find("2f 56 4d 43") != std::string::npos);

    // A payload byte 0x7c renders as '|' inside the gutter, so the gutter runs
    // to the last '|' on the line rather than the second.
    const std::string pipes = "!vmc-packet-capture 1\n"
                              "\n"
                              "d 0.000000 3\n"
                              "  7c 41 7c  ||A||\n";
...
{
    const std::string header = "!vmc-packet-capture 1\n";
    const BadCapture cases[] = {
        {"no magic", "sender example\n", 1},
        {"unsupported version", "!vmc-packet-capture 2\n", 1},
        {"junk after the version", "!vmc-packet-capture 1 extra\n", 1},
        {"unknown header key", header + "provider example\n", 2},
        {"header key with no value", header + "sender\n", 2},
        {"duplicated header key", header + "sender a\nsender b\n", 3},
...
{
    // A DCC that calls setlocale is the realistic case, and `d 0,033333` would
    // be a capture this reader refuses -- written by this writer.
    PacketCapture capture;
    capture.datagrams.push_back(Datagram(0.033333, {0x2f}));
    ...
    assert(written.find("d 0.033333 1\n") != std::string::npos);
    PacketCapture parsed;
...
CheckCorpus(const std::filesystem::path& directory)
{
    ...
    for (const std::filesystem::path& path : captures)
    {
        ...
        // carry: a committed capture that names neither its sender nor its
    ...
    std::printf("VMC packet corpus: %zu capture(s) verified\n", captures.size());
    return 0;
}
