...
| --- | --- | --- | --- | --- | --- |
| v0.1.0: contract and every imported input | `motionConnectorCore` (`IMotionConnector`, `MotionFrame`, state, capabilities, timing, the bounded buffer); source profiles; `motionConnectorTransport`, `motionConnectorOsc`, `motionConnectorTracking`; `motionConnectorVmc`, `motionConnectorMocopi`, `motionConnectorVrchatOsc`; `motion_connect dump`, `list`, `inspect`; generated corpus; hardware-free CI | `liveTransport`, `osc`, `motionTracking`, `vrmAdapterVmc`, `vrmAdapterMocopi`, `vrmAdapterVrchatOsc`, `vmc_record`, `mocopi_record`, `vrchat_osc_record` | 1, 2, and 6 as an import | `usd-motion-plugins` v0.1.0 (`motionCore`, `motionRecording` installed) | ⛔ |
| v0.2.0: transport | `motionConnectorWebSocket`; `motion_connect record`, `bridge`; `examples/record_stream`; Python bindings | none | 3 | v0.1.0 | ⬜ |
...
- **mocopi moves from v0.4.x to v0.1.0, and VRChat OSC Trackers is added
  there.** Both are imports of measured code, not new designs. They were first
  planned for v0.2.0. WS-O7 (decided 2026-09-19) moved every live input into
  v0.1.0, because `liveTransport` and `osc` are linked by all three connectors,
  and importing them first would leave `usd-vrm-plugins` two copies for a
  release or an undeclared edge to this repository. §30's concern, that mocopi
  should not define the core API, is still met. `motionConnectorCore` exists
  before any import, and each connector adapts to it after it moves
...
- **`motionConnectorOsc` is the OSC wire format, not a connector.** §31 lists
  ...
  reopens it.
- **v0.1.0 needs OpenUSD's foundation types**, through `motionCore`
...
| CS-O1 | Where the change-of-basis primitive lives | [COORDINATES §6](../design/COORDINATE_SYSTEMS.md#6-open-questions) | the VRChat OSC import, v0.1.0 |
| CS-O3 | Conversion as code or profile data | [COORDINATES §6](../design/COORDINATE_SYSTEMS.md#6-open-questions) | CS-O1 |
| WS-O2 | Where the tracker solve lives | [WORKSPACE §7](../architecture/WORKSPACE.md#7-open-questions) | the VRChat OSC import, v0.1.0 |
| CC-O4 | Per-joint tracking loss (feeds MC-O6) | [CONNECTOR §13](../design/CONNECTOR_CONTRACT.md#13-open-questions) | the mocopi import, v0.1.0 |
...
| WS-O3 | Record tools vs `motion_connect record` | [WORKSPACE §7](../architecture/WORKSPACE.md#7-open-questions) | after the last import |
